...
//    	System.out.println(datalink);
    ...
    public ModelAndView appliedinternship(HttpSession session)
    {
    	...
    	ModelAndView mv = new ModelAndView();
    	List<ApplyIntenships> appliedIntenships=customerService.getAllAppliedInternships(customer.getId());
    	List<WorkSubmition> workSubmition=customerService.getWorkSubmissionsByStudent(customer.getId());
    ...
    public ModelAndView viewDetailsOfInternShip(@RequestParam("id") int id,HttpSession session)
    {
    	Customer customer=(Customer)session.getAttribute("customer");
    	ModelAndView mv = new ModelAndView();
    	ApplyIntenships internshipdetails=customerService.getAppliedInternshipsByID(id);
    	List<WorkSubmition> workSubmition=customerService.getWorkSubmissionsByStudent(customer.getId());
    	for(WorkSubmition a:workSubmition)
    	{
//    		System.out.println(a);
    		if(internshipdetails.getId()==a.getApplyIntenships().getId())
    		{
//    			System.out.println(internshipdetails.getId());
    			mv.addObject("submissionstatus","True" );
    		}
    	}
    ...
    @GetMapping("/viewinterndetails")
    public ModelAndView viewinterndetails(@RequestParam("id") int id)
    {
    	ModelAndView mv = new ModelAndView();
    	Internship internshipdetails=customerService.getInternById(id);//=customerService.getAppliedInternshipsByID(id);
    	 mv.addObject("internship",internshipdetails );
         mv.setViewName("viewinterndetails");
         return mv;
    }
